...
resource "aws_lambda_function" "the_patient_creator_lambda_function" {
  function_name    = local.patient_creator_pre_fixed_lambda_name
  handler          = "${local.patient_creator_pre_fixed_lambda_name}.lambda_handler"
...
resource "aws_cloudwatch_log_group" "the_patient_creator_lambda_cloudwatch_group" {
  name              = local.patient_creator_pre_fixed_lambda_name
  retention_in_days = 90
}

resource "aws_iam_role" "the_patient_creator_lambda_role" {
  name               = "${local.patient_creator_pre_fixed_lambda_name}_lambda_role"
  path               = "/"
...
resource "aws_iam_policy" "the_patient_creator_lambda-execution_policy" {
  name   = "${local.patient_creator_pre_fixed_lambda_name}_lambda_execution_policy"
...
data "archive_file" "the_patient_creator_lambda_zip" {
  ...
  source {
    ...
    filename = "${local.patient_creator_pre_fixed_lambda_name}.py"
  }
}
...
# Module Locals
locals {
  patient_creator_lambda_name                 = "creator_lambda"
  patient_creator_pre_fixed_lambda_name       = "${var.patient-prefix}${local.patient_creator_lambda_name}"
  patient_creator_lambda_function_source_path = "${path.module}/lambda/${local.patient_creator_lambda_name}.py"
  patient_creator_lambda_function_output_path = "${path.module}/lambda/${local.patient_creator_pre_fixed_lambda_name}.zip"
}