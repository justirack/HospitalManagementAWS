...
resource "aws_api_gateway_gateway_response" "the_patient_rest_api_gateway_response" {
  ...
  response_parameters = {
    "gatewayresponse.header.Access-Control-Allow-Origin" = "'${local.patient_api_cors_domain}'"
  }
...
data "template_file" "the_patient_open_api_specification_file" {
  ...
  vars = {
    ...
    delete_path        = local.patient_api_delete_path
    delete_description = local.patient_api_delete_description

    # Options endpoint variables
    options_path        = local.patient_api_options_path
    options_description = local.patient_api_options_description

    # Validation lambda information
    patient_validation_lambda_invoke_arn      = aws_lambda_function.the_patient_validation_lambda_function.invoke_arn
    patient_validation_lambda_invoke_role_arn = aws_iam_role.the_patient_validation_lambda_role.arn

    # Options lambda information
    patient_options_lambda_invoke_arn      = aws_lambda_function.the_patient_options_lambda_function.invoke_arn
    patient_options_lambda_invoke_role_arn = aws_iam_role.the_patient_options_lambda_role.arn
...
locals {
  ...
  patient_api_path_version = "v1"
  patient_api_invoke_url   = "https://${local.patient_api_title}"

  # Add endpoint variables
  patient_api_add_path        = "${local.patient_api_path_version}/${local.patient_api_title}/add"
  patient_api_add_description = "The endpoint that adds a patient to the database."

  # retrieve endpoint variables
  patient_api_retrieve_path        = "${local.patient_api_path_version}/${local.patient_api_title}/get"
  patient_api_retrieve_description = "The endpoint that gets a patient from the database"

  # Update endpoint variables
  patient_api_update_path        = "${local.patient_api_path_version}/${local.patient_api_title}/update"
  patient_api_update_description = "The endpoint that updates an existing patients information."

  # Delete endpoint variables
  patient_api_delete_path        = "${local.patient_api_path_version}/${local.patient_api_title}/delete"
  patient_api_delete_description = "The endpoint that deletes a patients information"

  # Options endpoint variables
  patient_api_options_path        = "${local.patient_api_path_version}/${local.patient_api_title}/options"
  patient_api_options_description = "An options endpoint for the patient API"
